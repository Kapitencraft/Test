...
public class JavaLexer implements Lexer {
    ...
    static {
        keywords = Arrays.stream(values()).filter(tokenType -> tokenType.isCategory(TokenTypeCategory.JAVA_KEY_WORD)).collect(Collectors.toMap(tokenType -> tokenType.name().toLowerCase(Locale.ROOT), Function.identity()));
    }
    ...
    public JavaLexer(String source, ErrorStorage errors) {
        this.source = source;
        this.errors = errors;
    }
